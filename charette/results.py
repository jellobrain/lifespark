...
def fullmonty(user, password):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT pc.member_id, a.first_name, a.last_name, pc.total_cost "
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT distinct YEAR(a.admission_date) as 'year', "
                   "date_format(a.admission_date, '%U') as 'week', "
                   "count(a.member_id) as 'num_admissions', sum(c.cost) as 'total_cost' "
                   "FROM admissions a, codes c WHERE a.code = c.code "
                   "group by YEAR(a.admission_date), "
                   "date_format(a.admission_date,'%U') "
                   "order by YEAR(a.admission_date) desc, week")
    ...
    cursor = conn.cursor()
    cursor.execute("WITH individual_admissions (member_id, admission_date) as "
